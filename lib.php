...
function theme_ey_get_pre_scss($theme) {
...
// Note the function name starts with the component name because this is a global function and we don't want namespace clashes.
function theme_ey_get_extra_scss($theme) {
    // Load the settings from the parent.                                                                                           
    $theme = theme_config::load('boost');                                                                                           
...
function theme_ey_get_main_scss_content($theme) {                                                                                
    global $CFG;                                                                                                                    
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_ey', 'preset', 0, '/', $filename))) {              
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/ey/scss/pre.scss');                                                         
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.                                    
    $post = file_get_contents($CFG->dirroot . '/theme/ey/scss/post.scss');                                                       
